...
open class YearWheelPicker: UIPickerView {
    ...
    open var minimumDate: Date {
        ...
        get {
            return _minimumDate ?? formattedDate(from: calendar.date(byAdding: .year, value: -15, to: Date()) ?? Date())
        }
    }
...
//    }
    ...
    open var onDateSelected: ((_ year: Int) -> Void)?
       ...
       // let month = calendar.component(.month, from: date)
        ...
        DispatchQueue.main.async {
            ...
            if let firstYearIndex = self.years.firstIndex(of: year) {
                self.selectRow(firstYearIndex, inComponent: 0, animated: animated)
            }
        }
    }
    ...
    private func pickerViewDidSelectRow() {
      //  let month = selectedRow(inComponent: 0) + 1
        ...
        guard let date = DateComponents(calendar: calendar, year: year, month: 1, day: 1, hour: 0, minute: 0, second: 0).date else {
    ...
    private func formattedDate(from date: Date) -> Date {
        return DateComponents(calendar: calendar, year: calendar.component(.year, from: date), month: 1, day: 1, hour: 0, minute: 0, second: 0).date ?? Date()
    }
...
extension YearWheelPicker: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    public func pickerView(_ pickerView: UIPickerView, attributedTitleForRow row: Int, forComponent component: Int) -> NSAttributedString? {
        ...
        let year = years[selectedRow(inComponent: 0)]
        if let date = DateComponents(calendar: calendar, year: year, month: 1, day: 1, hour: 0, minute: 0, second: 0).date, date < minimumDate || date > maximumDate {
            if #available(iOS 13.0, *) {
